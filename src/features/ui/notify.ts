// src/features/ui/notify.ts
// Enkel toast-notifier med auto-dismiss och inbyggd CSS
...
function getContainer(): HTMLElement {
  let el = document.querySelector('.notify-container') as HTMLElement | null;
  if (!el) {
...
function show(kind: 'success' | 'error', message: string, opts?: NotifyOptions) {
  ensureStyles();
...
export function showSuccess(message: string, opts?: NotifyOptions) {
  show('success', message, opts);
}

export function showError(message: string | unknown, opts?: NotifyOptions) {
  const msg =
    typeof message === 'string'
      ? message
      : (message && (message as any).message) || 'Ett fel inträffade';
  show('error', msg, opts);
}
